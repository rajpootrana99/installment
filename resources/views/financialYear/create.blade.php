                ...
                <div class="col-sm-12">
                    <div class="iq-card">
                        ...
                        <div class="iq-card-body">
                            <form method="post" action="{{route('financialYear.store')}}">
                                @csrf
                                <div class="row">
                                    <div class="form-group col-6">
                                        <label for="start_date">Start Date</label>
                                        <input type="date" class="form-control" id="exampleInputdate" name="start_date" value="{{ old('start_date') }}">
                                        <div style="color: #ff0000; font-size: x-small; margin-top: 3px;">{{ $errors->first('start_date') }}</div>
                                    </div>
                                    <div class="form-group col-6">
                                        <label for="end_date">End Date</label>
                                        <input type="date" class="form-control" id="exampleInputdate" name="end_date" value="{{ old('end_date') }}">
                                        <div style="color: #ff0000; font-size: x-small; margin-top: 3px;">{{ $errors->first('end_date') }}</div>
                                    </div>
                                    <div class="form-group col-6">
                                        <label for="year_string">Year String:</label>
                                        <input type="text" name="year_string" class="form-control" placeholder="Enter Year String" value="{{ old('year_string') }}">
                                    ...
                                    <div class="form-group col-6">
                                        ...
                                        <select name="status" class="form-control">
                                            @foreach($financialYear->statusOptions() as $statusOptionKey => $statusOptionValue)
                                                <option value="{{ $statusOptionKey }}">{{ $statusOptionValue }}</option>
                                            @endforeach
                                        </select>
