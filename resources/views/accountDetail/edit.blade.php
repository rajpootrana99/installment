...
@section('section')
    ...
    <div id="content-page" class="content-page">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-12">
                    <form method="post" action="{{route('accountDetail.update', ['accountDetail' => $accountDetail])}}" enctype="multipart/form-data">
                        @method('PATCH')
                        @csrf
                        <div class="iq-card">
                            ...
                            <div class="iq-card-body">
                                <div class="form-group">
                                    ...
                                    <select name="head_id" class="form-control">
                                        @foreach($heads as $head)
                                            <option value="{{ $head->id }}"{{ $accountDetail->head_id == $head->id ? 'selected' : '' }}>{{ $head->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="sub_head_id" class="form-control">
                                        @foreach($subHeads as $subHead)
                                            <option value="{{ $subHead->id }}"{{ $accountDetail->sub_head_id == $subHead->id ? 'selected' : '' }}>{{ $subHead->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="account_detail_name" class="form-control" placeholder="Enter Account Detail Name" value="{{ $accountDetail->account_detail_name }}">
                                    <div style="color: #ff0000; font-size: x-small; margin-top: 3px;">{{ $errors->first('account_detail_name') }}</div>
                                </div>
                                <div class="form-group">
                                    <input type="hidden" name="serial_number" class="form-control" value="{{ $accountDetail->serial_number }}" readonly>
                                </div>
                            </div>
                        </div>
                        <div class="iq-card">
                            ...
                            <div class="iq-card-body">
                                <div class="form-group row">
                                    @foreach($accountDetail->accountNatureOptions() as $accountNatureOptionKey => $accountNatureOptionValue)
                                        <div class="col-6">
                                            <input type="radio" name="account_nature" value="{{ $accountNatureOptionKey }}"{{ $accountDetail->account_nature == $accountNatureOptionValue ? 'checked' : '' }} id="radio1">
