                ...
                <div class="col-sm-12">
                    <div class="iq-card">
                        ...
                        <div class="iq-card-body">
                            <form method="post" action="{{route('subHead.update', ['subHead' => $subHead])}}" enctype="multipart/form-data">
                                @method('PATCH')
                                @csrf
                                <div class="form-group">
                                    ...
                                    <select name="head_id" class="form-control">
                                        @foreach($heads as $head)
                                            <option value="{{ $head->id }}"{{ $subHead->head_id == $head->id ? 'selected' : '' }}>{{ $head->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control" placeholder="Name" value="{{ $subHead->name }}">
                                    <div style="color: #ff0000; font-size: x-small; margin-top: 3px;">{{ $errors->first('name') }}</div>
                                </div>
                                <div class="form-group">
                                    <label for="serial_code">Serial Code:</label>
                                    <input type="text" name="serial_code" class="form-control" placeholder="Enter Serial Code" value="{{ $subHead->serial_code }}">
                                    <div style="color: #ff0000; font-size: x-small; margin-top: 3px;">{{ $errors->first('serial_code') }}</div>
                                </div>
